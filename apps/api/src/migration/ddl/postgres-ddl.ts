...
export function buildCreateTable(
  ...
  for (const col of schema.columns) {
    ...
    // Auto-increment identity (PG ≥10). GENERATED BY DEFAULT lets bulk loads
    // supply explicit ids; sequence is resynced in PostgresWriter.finalize().
    // Only apply to integer columns; widen to BIGINT for 'long'.
    const identityClause =
      col.autoIncrement && (col.type === 'int' || col.type === 'long')
        ? ' GENERATED BY DEFAULT AS IDENTITY'
        : '';
    colDefs.push(`  "${cname}" ${sqlType}${nullClause}${identityClause}`);
